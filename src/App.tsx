...
const load = (): AppData => { try { const saved = localStorage.getItem(key); if (!saved) return initialData(); const value = JSON.parse(saved); if (value?.version === 3 && Array.isArray(value.weeks)) return value; if (value?.version === 2 && Array.isArray(value.weeks)) return { version: 3, weeks: value.weeks, sessions: topicSessions(value.weeks), currentTopicId: value.currentTopicId, theme: value.theme }; const fresh = initialData(); return { ...fresh, sessions: Array.isArray(value?.sessions) ? value.sessions : [], theme: value?.theme }; } catch { return initialData(); } };
